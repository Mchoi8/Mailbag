...
export function createState(inParentComponent) {
  ...
  return {
    ...
    /**
     * Hide/Show contact list when clicked
     */
    contactToggle : function(): void {

      if(this.state.contactCheck){

        if(this.state.mailboxCheck == false) {
          this.setState({ contactCheck: false, gridPos: 4});
        } else {
          this.setState({ contactCheck: false, gridPos: 2});
        }

      } else {
          this.setState({ contactCheck: true, gridPos: 0 });
        }
    ...
    /**
     * Hide/Show mailbox list when clicked
     */
    mailboxToggle : function(): void {      

      if(this.state.mailboxCheck){

        if(this.state.contactCheck == false) {
          this.setState({mailboxCheck: false , gridPos: 4});
        } else {
          this.setState({ mailboxCheck: false});
        }

      } else {
          this.setState({ mailboxCheck: true, gridPos: 0 });
      }
    ...
    showHidePleaseWait : function(inVisible: boolean): void {
      ...
      this.setState({ dialogToggle : inVisible });
     ...
     * Go Home Button
     */
    goHome : function(currView: String): void {
      this.setState({ currentView : currView });
    }.bind(inParentComponent), /* End showContact(). */
     ...
     */
    showContact : function(inID: string, inName: string, inNumber: string): void {
      this.setState({ currentView : "contact", contactID : inID, contactName : inName, contactNumber : inNumber });
    ...
    showAddContact : function(): void {
      this.setState({ currentView : "contactAdd", contactID : null, contactName : "", contactNumber : "" });
    ...
    addMailboxToList : function(inMailbox: IMAP.IMailbox): void {
      ...
      // Copy list.
      const mbList: IMAP.IMailbox[] = this.state.mailboxes.slice(0);

      // Add new element.
      mbList.push(inMailbox);

      // Update list in state.
      this.setState({ mailboxes : mbList });
    ...
    addContactToList : function(inContact: Contacts.IContact): void {
      ...
      // Copy list.
      const contactList = this.state.contacts.slice(0);
      // Add new element.
      if(inContact.name != undefined) {
        contactList.push({ _id : inContact._id, name : inContact.name, phoneNumber : inContact.phoneNumber });
      }
    ...
    addMessageToList : function(inMessage: IMAP.IMessage): void {
      ...
      // Copy list.
      const msgList = this.state.messages.slice(0);

      // Add new element.
      msgList.push({ id : inMessage.id, date : inMessage.date, from : inMessage.from, subject : inMessage.subject });
    ...
    saveContact : async function(): Promise<void> {
      ...
      console.log("state.saveContact()", this.state.contactID, this.state.contactName, this.state.contactNumber);
      ...
      const contact: Contacts.IContact =
        await contactsWorker.addContact({ name : this.state.contactName, phoneNumber : this.state.contactNumber });
      this.state.showHidePleaseWait(false);
      ...
      // Update state
      this.setState({ contacts : cl, contactID : null, contactName : "", contactNumber : "" });
    ...
    updateContact : async function(): Promise<void> {
      ...
      const contactsWorker: Contacts.Worker = new Contacts.Worker();
      const contact: Contacts.IContact =
        await contactsWorker.updateContact({ name : this.state.contactName, phoneNumber : this.state.contactNumber });
      this.state.showHidePleaseWait(false);

      for(let i=0; i<contactList.length; i++){
        if(contactList[i].name == this.state.contactName){
          contactList[i].phoneNumber = this.state.contactNumber;
        }
      }

      // Update state for the contact view
      this.setState({ contacts : contactList, contactID : null, contactName : "", contactNumber : "" });
    ...
    deleteContact : async function(): Promise<void> {
      ...
      // Remove from list.
      const contactList = this.state.contacts.filter((inElement) => inElement._id != this.state.contactID);
    ...
    deleteMessage : async function(): Promise<void> {
      ...
      // Remove from list.
      const msgList = this.state.messages.filter((inElement) => inElement.id != this.state.messageID);

      // Update state.
      this.setState({ messages : msgList, currentView : "welcome" });
